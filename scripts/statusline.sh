...
#   Line 1: api-domain │ project │ branch │ context% │ timestamp │ diff stats  (wraps naturally)
#   Line 2: session summary  (always on its own line below)
#
...
# Parse all needed fields in a single Node.js invocation (one line per field)
parsed=$(echo "$input" | node "$PARSE_JSON" session_id context_window.used_percentage workspace.project_dir workspace.current_dir transcript_path)
session_id=$(echo "$parsed" | sed -n '1p')
context_pct=$(echo "$parsed" | sed -n '2p')
project_dir=$(echo "$parsed" | sed -n '3p')
current_dir_fallback=$(echo "$parsed" | sed -n '4p')
...
# Fallback: project_dir from API (if valid directory) > current_dir from API
...
# Detect git branch
# Use git -C instead of checking .git directory (supports worktrees)
branch=""
if [ -n "$initial_dir" ]; then
    branch=$(git -C "$initial_dir" symbolic-ref --short HEAD 2>/dev/null || echo "")
fi
...
if [ -n "$transcript_path" ] && [ -f "$transcript_path" ]; then
    ...
    # Try macOS date -r first, then Linux date -d (separate commands so || works)
    if [ -n "$use_12_hour" ]; then
        # 12-hour format: 3:42pm Feb 5
        formatted=$(date -r "$mtime" "+%-l:%M%p %b %-d" 2>/dev/null)
        [ -z "$formatted" ] && formatted=$(date -d "@$mtime" "+%-l:%M%p %b %-d" 2>/dev/null)
        formatted=$(echo "$formatted" | sed 's/AM/am/g; s/PM/pm/g')
    else
        # 24-hour format: 15:42 Feb 5
        formatted=$(date -r "$mtime" "+%-H:%M %b %-d" 2>/dev/null)
        [ -z "$formatted" ] && formatted=$(date -d "@$mtime" "+%-H:%M %b %-d" 2>/dev/null)
    fi
...
diff_display=""
if [ -n "$initial_dir" ] && [ -n "$branch" ]; then
    diff_stats=$(git -C "$initial_dir" diff --shortstat 2>/dev/null)
...
# Output
if [ -n "$summary" ]; then
    printf "%s\n\033[33m▸ %s\033[0m" "$line1" "$summary"
else
    printf "%s" "$line1"
fi
